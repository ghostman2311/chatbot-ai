import { onGetAllBookingForCurrentUser } from "@/actions/appointment";
...
const AppointmentPage = async (props: Props) => {
  const user = await currentUser();

  if (!user) return null;

  const domainBookings = await onGetAllBookingForCurrentUser(user.id);
  const today = new Date();
  console.log(domainBookings, "domainBookings");
